...
def main(prompt, directory=DEFAULT_DIRECTORY, model=DEFAULT_MODEL, file=None):
    ...
    generator.generate_readme_and_docs()
    ...
    # Update README.md with the program structure
    readme = generator.update_readme(program_structure)

    initial_dependencies = generator.generate_shared_dependencies(
        program_structure, readme
    )

    # Generate file paths
    filepaths = generator.generate_file_paths(
        program_structure, initial_dependencies, readme
    )
    print(filepaths)

    # Generate shared dependencies
    shared_dependencies = generator.refine_shared_dependencies(
        filepaths, program_structure
    )

    # Update DOCUMENTATION.md with the shared dependencies
    generator.update_documentation(shared_dependencies, filepaths)

    # Generate code for each file and write to directory
    for filepath in filepaths:
        filecode, unit_tests = generator.generate_code_for_each_file(
            filepath, program_structure, shared_dependencies
        )
        # Debug generated code
        generator.debug_generated_code()
        generator.update_readme_for_code(filecode)
        generator.update_documentation_for_code(filecode)
